...
1. 01_pydantic_schema.py
   使用 Pydantic BaseModel 和 Field 定义客户分析结果，并结合查询、邮件工具生成
   经过运行时校验的结构化对象。

2. 02_dataclass_schema.py
   使用 Python dataclass、类型标注和 field(metadata=...) 定义 Schema，
   并通过 dataclasses.asdict() 将结果转换为字典。

3. 03_typed_dict_schema.py
   使用 TypedDict、Annotated 和 Literal 描述字典结构；通过 total=False
   允许字段省略，适合返回普通 dict 的场景。

4. 04_json_schema.py
   直接使用 JSON Schema 字典配置 properties、enum、required 和
   additionalProperties，不依赖 Python 数据模型类。

5. 05_custom_structured_output_message.py
   使用 tool_message_content 自定义结构化输出成功后写入消息历史的 ToolMessage，
   同时配置 handle_errors=False，说明如何关闭结构化输出错误的自动重试。
