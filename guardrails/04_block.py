...
# 工具定义
@tool
def get_user_info(email: str, credit_card: str, server_ip: str, api_key: str) -> str:
    """模拟获取用户信息工具"""
    print("\n" + "=" * 60)
    user_info = f"用户信息: 用户的邮箱为 {email}, 信用卡号为 {credit_card}, 服务器地址为 {server_ip}, api_key为 {api_key}"
...
result = agent.invoke({  # type: ignore
    "messages": [
        {
            "role": "user",
            "content": "我的邮箱是 zhangsan@company.com，"
                       "信用卡号是 5105-1051-0510-5100，"
                       "服务器地址是 192.168.1.1，"
                       "我的api_key是sk-abc123def456ghi789jkl012mno345pqr，"
                       "请输出我的信息"
        }
    ]
})
